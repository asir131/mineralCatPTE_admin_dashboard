...
// Global mock test question ID key
...
export default function RepeatSentence() {
  ...
  // On initial mount, load selectedIds from global "mock_full_question_ids" if mockTest
  useEffect(() => {
    if (isMockTest) {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/speaking/repeat_sentence?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/read-aloud/${item._id}`, {
      state: {
        ...
        api: "/test/speaking/repeat_sentence",
      },
    });
  };
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Speaking (Repeat Sentence)</h1>
          </button>
        </div>
      ...
      <div className="p-4">
        {isLoading ? (
          ...
          <div className="space-y-3">
            {data && data.length > 0 ? (
              data.map((item) => {
                // For mockTest, highlight selected and remove 3dots
