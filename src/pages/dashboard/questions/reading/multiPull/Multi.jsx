...
export default function Multi() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/reading/mcq_multiple?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/test/listening/multiple-choice-multiple-answers/${item._id}`, {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/reading/mcq_multiple", // Correct API URL
  ...
  const handleClick = () => {
    navigate("/question/multiple-choice-reading/add", {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/reading/mcq_multiple", // Correct API URL
      },
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Reading (Multiple Choice - Multiple Answers)</h1>
          </button>
        </div>
