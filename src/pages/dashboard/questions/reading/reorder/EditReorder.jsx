...
export default function EditReorder() {
  ...
  const api = "/test/reading/reorder-paragraphs"
  ...
  useEffect(() => {
    if (location.state) {
      ...
      if (location.state.prompt) {
        setHeading(location.state.prompt)
      }
  ...
  const validateForm = () => {
    ...
    if (options.some((option) => !option.text.trim())) {
      toast.error("Please fill all sentence fields")
      return false
    }

    // Check minimum number of sentences
    if (options.length < 2) {
      toast.error("Please add at least 2 sentences to reorder")
      return false
    }
  ...
  const handleUpdate = () => {
    ...
    fetchWithAuth(`${baseUrl}${api}`, {
      ...
      .then((data) => {
        ...
        setLoading(false)
        // Navigate to the reorder paragraphs page
        navigate("/question/reorder-paragraphs")
      })
      .catch((error) => {
      ...
      <div className="p-4 max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <label className="block text-gray-700 text-sm font-medium">Input Source</label>
            <button
              onClick={addOption}
          ...
          <div className="space-y-3">
            {options.map((option, index) => (
              <div key={option.id} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200">
                {/* Letter Circle */}
                <div
                  className={`w-8 h-8 rounded-full ${getOptionColor(option.letter)} flex items-center justify-center text-white font-medium text-sm flex-shrink-0`}
                >
                  {option.letter}
                </div>
          ...
          <div className="mt-2 text-xs text-gray-500">
            <p>💡 Tip: Enter sentences in the correct order. Students will need to rearrange them.</p>
          </div>
        </div>
