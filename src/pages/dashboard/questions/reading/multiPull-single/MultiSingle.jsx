...
export default function MultiSingle() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/reading/mcq_single?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/multiple-choice-single-reading/${item._id}`, {
      state: {
        from: location.pathname,
        api: "/test/reading/mcq_single",
        uniquePart: item._id,
      },
    });
  };
  ...
  const handleClick = () => {
    navigate("/question/multiple-choice-single-reading/add", {
      state: {
        from: location.pathname,
        api: "/test/reading/mcq_single",
      },
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Reading (Multiple Choice - Single Answer)</h1>
          </button>
        </div>
