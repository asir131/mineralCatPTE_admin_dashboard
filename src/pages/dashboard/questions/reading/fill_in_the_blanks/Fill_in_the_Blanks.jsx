...
export default function Fill_in_the_Blanks() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/reading/fill-in-the-blanks?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/fill-blanks-reading/${item._id}`, {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/reading/fill-in-the-blanks", // Corrected API URL
  ...
  const handleClick = () => {
    navigate("/question/fill-blanks-reading/add", {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/reading/fill-in-the-blanks", // Corrected API URL
      },
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Reading (Fill in the blanks)</h1>
          </button>
        </div>
