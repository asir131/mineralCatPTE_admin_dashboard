...
export default function Fill_the_single() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/listening/listening-fill-in-the-blanks?query=all&page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/fill-blanks-listening/${item._id}`, {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/listening/listening-fill-in-the-blanks", // API URL
        uniquePart: item._id, // Unique part for the question
        type: "listening", // Specify the type for the new question
        subtype: "listening_fill_in_the_blanks", // Specify the subtype for the new question
      },
    });
  };
  ...
  const handleClick = () => {
    navigate("/question/fill-blanks-listening/add", {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/listening/listening-fill-in-the-blanks", // API URL
        type: "listening", // Specify the type for the new question
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Listening (Fill in the Blanks)</h1>
          </button>
        </div>
