...
export default function Summarize_spoken() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/listening/summarize-spoken-text?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/summarize-spoken/${item._id}`, {
      state: {
        from: location.pathname,
        api: "/test/listening/summarize-spoken-text",
        uniquePart: item._id,
      },
    });
  };
  ...
  const handleClick = () => {
    navigate("/question/summarize-spoken/add", {
      state: {
        from: location.pathname,
        api: "/test/listening/summarize-spoken-text",
        type: "listening",
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate("/question")} className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Listening (Summarize Spoken Text)</h1>
          </button>
        </div>
