...
export default function Multiple_single() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/listening/multiple-choice-single-answers?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/multiple-choice-listening/${item._id}`, {
      state: {
        ...
        api: "/test/listening/multiple-choice-single-answers", // API URL
        uniquePart: item._id, // Unique part for the question
        type: "listening", // Specify the type for the new question
        subtype: "listening_multiple_choice_single_answers", // Specify the subtype for the new question
      },
    });
  };
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Listening (Multiple Choice - Single Answer)</h1>
          </button>
        </div>
