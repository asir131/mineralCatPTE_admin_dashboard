...
// Global mock test key for all mock test question id selection
...
export default function EmailWrite() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/writing/write_email?query=all&page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/read-aloud/${item._id}`, {
      state: {
        ...
        api: "/test/writing/write_email", // API URL
  ...
  const handleClick = () => {
    navigate("/question/read-aloud/add", {
      state: {
        from: location.pathname, // Current page URL
        api: "/test/writing/write_email", // API URL
        subtype: "write_email", // Specify the subtype for the new question
        type: "writing", // Specify the type for the new question
      },
    });
  };
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
         <button className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-medium">Writing (Write Email)</h1>
         </button>
        </div>
