...
export default function SummarizeWritten() {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(
        `${baseUrl}/test/writing/summarize-written-text?page=${currentPage}&limit=${itemsPerPage}`
      );
  ...
  const handleEdit = (item) => {
    navigate(`/question/read-aloud/${item._id}`, {
      state: {
        ...
        api: "/test/writing/summarize-written-text", // API URL
  ...
  const handleClick = () => {
    navigate("/question/read-aloud/add", {
      state: {
        ...
        subtype: "summarize_written_text", // Specify the subtype for the new question
      },
    });
  };
      ...
      <div className="bg-red-700 text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-medium">Writing (Summarize written)</h1>
          </button>
        </div>
