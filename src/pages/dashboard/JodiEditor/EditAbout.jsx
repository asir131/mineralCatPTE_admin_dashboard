...
export default function EditAbout() {
  ...
  useEffect(() => {
    if (location.pathname.includes("/settings/about/edit")) {
      setDynamicAPIPath("/about-us/");
    } else {
      setDynamicAPIPath(""); 
    }
  }, [location.pathname]);
  ...
  useEffect(() => {
    ...
    const fetchInitialContent = async () => {
      try {
        ...
        setContent(data[0]?.aboutUsText || `<b>হ্যালো সাকিব</b>`);
      } catch (err) {
  ...
  const mutation = useMutation({
    ...
    //   if (!id) throw new Error("Term ID not set");
      ...
      const res = await fetchWithAuth(`${baseUrl}${dynamicAPIPath}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ aboutUsText: data, id }),
      });
    ...
    onSuccess: () => {
      ...
      navigate("/settings/about"); // redirect React-router এর মাধ্যমে
    },
    onError: (error) => {
