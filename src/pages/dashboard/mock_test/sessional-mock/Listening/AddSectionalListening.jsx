...
export default function AddSectionalListening() {
  ...
  // const api = location?.state?.api || "/test/mock/full";
  // const from = location?.state?.from || "/mock/full";

  const navigate = useNavigate();
 useEffect(() => {
    // JSON.parse দিয়ে লোকাল স্টোরেজের ডেটা পার্স করা
  ...
  const handleSubmit = async (e) => {
    ...
    const body = {
      name: mockName,
      type: "listening",
      // approxTime: `${hour.padStart(2, "0")}:${minute.padStart(2, "0")}`,
      duration:{
      hours: hour,
      minutes: parseInt(hour) * 60 + parseInt(minute),
      },
      // totalQuestions : qustions.length,
      questions: qustions,
      // type: location.state?.type || "mock",
      // subtype: location.state?.subtype || "full",
    };

    fetchWithAuth(`${baseUrl}/sectional-mock-test/add`, {
      method: "POST",
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <AllQuestions isMockTest={isMockTest} setQuestions={setQuestions} sectional={sectional} url={"/mock/listening-tests/add"} />
